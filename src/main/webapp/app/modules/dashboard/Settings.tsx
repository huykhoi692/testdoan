...
const Settings: React.FC = () => {
  ...
  const handleConfirmUpload = async () => {
    if (!previewImage) {
      message.warning(currentLocale === 'vi' ? 'Vui lòng chọn một ảnh để tải lên' : 'Please select an image to upload');
      return;
    }
    setUploading(true);
    try {
        ...
        <Form form={form} layout="vertical" onFinish={onFinish}>
                ...
                >
                  {previewImage && activeTab === 'paste' ? (
                    <Text type="success" strong>
                      <CheckCircleOutlined /> Ảnh đã được paste!
                    </Text>
                  ) : (
                    <>
                      <CopyOutlined style={{ fontSize: 48, color: ds.colors.primary.DEFAULT }} />
                      <p>Click và nhấn Ctrl+V để paste</p>
                    </>
                  )}
                </div>
              </Tabs.TabPane>
          ...
          <div className="mb-8">
            <Title level={4} style={{ color: ds.colors.text.primary, marginBottom: ds.spacing.md }}>
              <UserOutlined style={{ color: ds.colors.info }} /> Thông tin cá nhân
            </Title>
            <Row gutter={24}>
              <Col xs={24} sm={12}>
              ...
              <Col span={24}>
                <Form.Item label="Tiểu sử" name="bio">
                  <TextArea rows={4} placeholder="Kể về bạn..." maxLength={200} showCount style={ds.inputStyle} />
                </Form.Item>
              </Col>
            </Row>
          </div>
          ...
          <div className="mb-8">
            <Title level={4} style={{ color: ds.colors.text.primary, marginBottom: ds.spacing.md }}>
              <BulbOutlined style={{ color: ds.colors.warning }} /> Giao diện
            </Title>
            <Radio.Group
              value={currentTheme}
          ...
          <div className="mb-10">
            <Title level={4} style={{ color: ds.colors.text.primary, marginBottom: ds.spacing.md }}>
              <BellOutlined style={{ color: ds.colors.error }} /> Thông báo
            </Title>
